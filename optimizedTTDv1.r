...
##### THESIS HUMBERTO ARIZA #############################################
erupciones <-c(
               #'Olive_white_lapilli.csv',
               #Ngamatea_1.csv',
               #'Ngamatea_2.csv',
               #'Okupata_Pourahu.csv',
               #'White_Lapilli.csv',
               #'XXIX_(Akurangi)_compiled.csv',
               #'M1.csv',
               #'XXVIII_compiled___sediments.csv',
               #'XXVII_(Oruamatua)_compiled.csv',
               'XXVI_(Shawcroft).csv',
               #'pre_Sw.csv',
               #'XXV_Compiled.csv',
               #'XXIV_Compiled.csv',
               ...
               #'XIX_Compiled.csv',
               #'Lahar___fluvial.csv',
               #'XVIII_3_to_5___synsed.csv',
               #'XVIII_2__synsediments.csv',
               #'XVIII_1.csv',
               #'XXVII_compiled.csv',
               #'XVI.csv',
               #'XIII_2_compiled.csv',
               ...
               #'XI.csv',
               #'X.csv',
               #'IX_(Mangatoetoenui)_compiled.csv',
               #'Hokey_Pokey_compiled.csv'
               )
setwd("C:/Users/Humberto Ariza/OneDrive - Universidad de Los Andes/Tesis VF/code/generated/rdataset")	
for(erupcion in erupciones){
  td <- read.table(erupcion, header=TRUE, sep=";" )	
  sv <- read.table("centro.csv", header=TRUE, sep=";")
  ## Operando para calcular downwind distances,distances y crosswind distances
  td <- prep(sv, td, 135)		#Angulo 135°
  model <- sample.reg(td, 80000,80000)		# Creando el modelo a partir de la regresion linear simple del espesor con 80 km y 80 km 
  ...
  write.csv2(td, paste("./r/",erupcion))
  grid <- form.grid(td, 500, 0.1, 1.2)
  trend <- trendmodel(td, sv, grid, 135, model)									### Form the trend. Note the wind direction has to be set again for
...
setwd("C:/Users/Humberto Ariza/OneDrive - Universidad de Los Andes/Tesis VF/code/generated/rdataset")	
td <- read.table("XXVI_(Shawcroft).csv", header=TRUE, sep=";" )						### Read the thickness dataset
sv <- read.table("centro.csv", header=TRUE, sep=";")							### Read source vent location

td <- prep(sv, td, 135)													### Calculate the distances, downwind distances, and crosswind distances of the sample sites,
																### here the wind direction is 20 degree from north clockwise.

model <- sample.reg(td, 80000,80000)											### Form either the segmented or the simple linear regression model of thickness against
...
write.csv2(td, "./r/tdShawcroft.csv")
